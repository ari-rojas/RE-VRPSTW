...
public final class HeuristicMinCostLabelingPricingProblemSolver extends AbstractPricingProblemSolver<EVRPTW, Route, PricingProblem> {
	...
	protected List<Route> generateNewColumns() {
		...
		while (!existsElementaryRoute && !maxNeighborhoodSize){
			...
			if( newRoutes.isEmpty() && nonElementaryRoutes.isEmpty()) { existsElementaryRoute = true; pricingProblemInfeasible=true; this.objective=Double.MAX_VALUE; }
			else {
	...
	public ExactSolution charging_pricing(ArrayList<Label> labels){
		...
		return new ExactSolution(newRoutes, nonElementaryRoutes);
	...
	public class ExactSolution {
		...
		public ArrayList<Route> newRoutes;
		public ArrayList<Route> nonElementaryRoutes;

		public ExactSolution(ArrayList<Route> nR, ArrayList<Route> nER){
			this.newRoutes = nR;
			this.nonElementaryRoutes = nER;
		}
	}
