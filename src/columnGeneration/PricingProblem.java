...
public final class PricingProblem extends AbstractPricingProblem<EVRPTW> {
	...
	public Map<Integer, Double> fixByReducedCosts(long timeLimit, double UB, double LB){
		...
		for (int j = 1; j <= dataModel.C+1; j++){
			...
			for (PPArc arc: dataModel.PPgraph.incomingEdgesOf(PPvertices[dataModel.C1_startID+j].id)){
				...
				double min_rc = findMinimumRCPath(backwardSequences, forwardSequences, arc.routing_arc, arc.modifiedCost);
	...
	public void runForwardLabeling(long timeLimit) {
		...
		Label l0 = new Label(0, -this.dualCost, dataModel.Q, vertices[0].opening_tw, remain_energy, 0,new boolean[dataModel.C], new boolean[dataModel.C], new boolean[this.subsetRowCuts.size()], new HashSet<Integer>(this.subsetRowCuts.size()));
		...
		ArrayList<Integer> base_Seq = new ArrayList<>(Arrays.asList(419, 1041, 848, 229, 1008, 157, 4));
