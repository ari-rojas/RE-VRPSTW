...
public class customCG extends ColGen<EVRPTW, Route, PricingProblem> {
	...
	public void solve(long timeLimit) throws TimeLimitExceededException{
		...
		dataModel.currentMasterIteration = 0;
		...
		do{
			...
			dataModel.currentMasterIteration ++;
			//Check whether the boundOnMasterObjective exceeds the cutoff value
			/* if (dataModel.CUTSENABLED && !foundNewColumns){ 		//check for inequalities. This can only be done if the master problem hasn't changed (no columns can be added).
		...
		} while (dataModel.currentMasterIteration < dataModel.numMasterIterations);
	...
	protected List<Route> invokePricingProblems(long timeLimit) throws TimeLimitExceededException {
		...
		if (optimalBound) pricingProblems.get(0).frcRouteLabels.clear(); */

		// Add columns to the master problem
		/* if(!newColumns.isEmpty()){
			for(Route column : newColumns) master.addColumn(column);
		} */
		return newColumns;
	}
