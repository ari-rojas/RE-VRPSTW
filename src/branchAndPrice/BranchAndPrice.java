...
public final class BranchAndPrice extends AbstractBranchAndPrice<EVRPTW,Route,PricingProblem> {
	...
	protected CGResult solveNode(BAPNode<EVRPTW,Route> bapNode, long timeLimit) throws TimeLimitExceededException {
		...
		try {
			...
			cg = new customCG(dataModel, master, pricingProblems, solvers, pricingProblemManager, bapNode.getInitialColumns(), objectiveIncumbentSolution, bapNode.getBound(), bapNode.nodeID, this.chargingNodes.contains(bapNode.nodeID)); //Solve the node
		...
		if (foundBranches){
			...
			if (foundBranches) {
				...
				if (foundBranches){
					...
					// 4. Branching on the charging scheduling subgraph
